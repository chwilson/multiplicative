...
$$\tag {10} \frac {dx}{dt} =  \lambda x + f_t(\lambda,x)$$
...
pareto_data <- super %>% filter(State.x == "New York") %>% group_by(Date) %>%  filter(10^4*(cases/population)>2) %>% summarize(alpha_hat = n()/sum(log(10^4*(cases/population)/2)), se_alpha = alpha_hat/sqrt(n()), empMean = mean(10^4*(cases/population)), trueMean = alpha_hat/(alpha_hat-1), ratio_empTrue = trueMean/empMean) %>% filter(alpha_hat < 3)
...
$$\tag{13} CV(I) = \sqrt(\frac{1}{\alpha^2 (\alpha-2)})$$
