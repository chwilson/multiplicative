...
ggplot(super %>% filter(State.x %in% c("Michigan","Florida", "Louisiana", "New York"), County.x %in% c("Washtenaw","Alachua", "Miami-Dade","Wayne","Orleans", "New York City")), aes(x=Date, y = 10^4*(cases/population), color = State.x, group = interaction(State.x,County.x))) + geom_line() + geom_point(aes(shape=County.x)) + scale_x_date(breaks = function(x) seq.Date(from = min(x),   to = max(x), by = "2 days"))+
  theme_bw(base_size=15) +   xlab("") + ylab("Cases (#/10000)") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
